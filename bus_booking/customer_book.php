		...
		<div class="col-md-12">
			...
			<div class="form-group mb-2">
				...
				<select  class="form-control" id="status" name="status" value="<?php echo isset($bmeta['qty']) ? $bmeta['qty'] : '' ?>">
					<option value="1" <?php echo isset($bmeta['status']) && $bmeta['status'] == 1 ? "selected" : '' ?>>Booked</option>
					<option value="0" <?php echo isset($bmeta['status']) && $bmeta['status'] == 0 ? "selected" : '' ?>>Unconfirmed</option>
					</select>
			</div>
			<?php endif; ?>
			<?php else: ?>
